...
5. **Header image space reservation** — the `<img>` tag has `width="1020" height="510"` attributes, which modern browsers (Chrome 88+, Firefox 89+, Safari 15+) automatically use to derive `aspect-ratio: auto 1020 / 510`. Combined with `width: 100%; height: auto` in CSS, the browser reserves the exact vertical space before the image loads with no layout shift. No explicit `aspect-ratio` on the container is needed or used. The `<img>` uses `display: block` (corrected from non-standard `display: flex`).
